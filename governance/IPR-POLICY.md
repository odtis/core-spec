# ODTIS intellectual property policy

**Applies to:** all contributions to `odtis/` normative text 
**License:** [CC BY 4.0](../LICENSE)

**Project hub:** [Project hub](../project/README.md) | **Trademark:** [Trademark policy](TRADEMARK-POLICY.md)

---

## Normative specification text

ODTIS normative prose, registry entries, OpenAPI bundles (Annex A), conformance tests, and governance documents in this repository are licensed under **Creative Commons Attribution 4.0 International (CC BY 4.0)**.

Implementations **need not** open-source code to claim ODTIS conformance.

---

## Contributions

By submitting a pull request, patch, or RFC proposal to this repository, contributors represent that:

1. they have the right to submit the work;
2. the contribution is offered under **CC BY 4.0** for normative content;
3. they will disclose known patent claims that read on normative MUST requirements (email editors listed in [Maintainers](MAINTAINERS.md)).

A formal Contributor License Agreement (CLA) MAY be required when the ODTIS Foundation is incorporated (Phase C).

---

## Patents

ODTIS aims for **royalty-free implementation** of normative requirements, comparable to OpenID Foundation Final Specifications.

Patent holders SHOULD notify editors before a MUST that implicates their patents is accepted. Contested MUST changes defer to foundation legal counsel when available.

---

## Trademarks

"ODTIS", "ODTIS Certified", and related marks are governed by [Trademark policy](TRADEMARK-POLICY.md) (draft). Spec text remains CC BY 4.0; marks require separate permission.

---

## Relationship to papers and books

| Corpus | License | Normative? |
|--------|---------|------------|
| `odtis/` | CC BY 4.0 | **Yes** |
| P01-P18 papers | CC BY-NC-ND 4.0 (per paper) | Informative |
| Book 1-3 | Per edition | Informative / implementation |

Papers MUST NOT override ODTIS MUST text after v1.0 freeze.

---

## Related

- [Spec lifecycle stages](SPEC-STAGES.md)
- [How to cite](../publication/HOW-TO-CITE.md)
