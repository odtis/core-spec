        ...
        if data.get("spec_version") == version:
            ...
            for line in lines:
                ...
                for pat, _ in PATTERNS:
                    m = pat.match(line)
                    if m:
                        updated = f"{m.group(1)}{version}{m.group(3)}" if len(m.groups()) == 3 else f"{m.group(1)}{version}"
                        break
                        if updated != line:
                            changed = True
        ...
        if file_needs_update(path, version):
            ...
            if not args.check:
                ...
                if not args.check and stale:
                    subprocess.run([sys.executable, str(ROOT / "scripts/build-conformance-manifest.py")], cwd=ROOT, check=False)
