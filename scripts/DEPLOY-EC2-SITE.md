# Despliegue de ODTIS en [digitaltrustinfrastructure.org](https://digitaltrustinfrastructure.org)
...
│
└──HTTP:80──► EC2 Nginx ──► /var/www/digitaltrustinfrastructure.org/
```
...
| **EC2 + Nginx** | `listen 80` por dominio (`your-other-domain.example` + `digitaltrustinfrastructure.org`)         |
| **Tu Mac**      | `./scripts/build-site.sh` → `rsync`                             |


**MkDocs:** `site_url: https://digitaltrustinfrastructure.org` en `site/mkdocs.yml` (ya configurado).
...
- [ ] **Cloudflare SSL/TLS:** modo **Flexible** en la zona `digitaltrustinfrastructure.org` (igual que tu otro sitio en el mismo EC2)
- [ ] **EC2:** `/var/www/digitaltrustinfrastructure.org` creado
- [ ] **Nginx:** vhost `digitaltrustinfrastructure.org` con **solo** `listen 80` (sin bloque SSL)
- [ ] `sudo nginx -t && sudo systemctl reload nginx`
- [ ] **Build + rsync**
- [ ] [https://digitaltrustinfrastructure.org](https://digitaltrustinfrastructure.org) OK; your-other-domain.example sigue OK

---

## 1. Cloudflare - DNS

Dashboard → **DNS → Records** (zona `digitaltrustinfrastructure.org`):


| Type      | Name  | Content                              | Proxy   |
| --------- | ----- | ------------------------------------ | ------- |
| **A**     | `@`   | IP pública del EC2 (misma que tu otro vhost) | Proxied |
| **CNAME** | `www` | `digitaltrustinfrastructure.org`                          | Proxied |


**Comprobar:** el registro A en el dashboard muestra tu IP de origen. Con proxy activo, `dig digitaltrustinfrastructure.org` puede devolver IPs de Cloudflare; es normal.
...
**En Nginx para digitaltrustinfrastructure.org:**
...
El vhost de `your-other-domain.example` debería tener solo `listen 80`. Haz lo mismo para `digitaltrustinfrastructure.org`.

### Redirect www → apex (opcional)

Cloudflare → **Rules → Redirect Rules:** `www.digitaltrustinfrastructure.org/*` → `https://digitaltrustinfrastructure.org/$1` (301).
...
sudo mkdir -p /var/www/digitaltrustinfrastructure.org
sudo chown -R ec2-user:ec2-user /var/www/digitaltrustinfrastructure.org
sudo chmod 755 /var/www/digitaltrustinfrastructure.org
```
...
# drwxr-xr-x ... ec2-user ec2-user ... digitaltrustinfrastructure.org
```

> **Error rsync `Permission denied`:** el directorio lo creó `root` con `sudo mkdir` sin `chown`. Ejecuta `sudo chown -R ec2-user:ec2-user /var/www/digitaltrustinfrastructure.org` y repite el `rsync`.

Tras un `rsync` correcto, Nginx (usuario `nginx`) debe poder **leer** los archivos:

```bash
# En el EC2, después del rsync
sudo find /var/www/digitaltrustinfrastructure.org -type d -exec chmod 755 {} \;
sudo find /var/www/digitaltrustinfrastructure.org -type f -exec chmod 644 {} \;

# Amazon Linux: SELinux puede causar 403 aunque exista index.html
sudo chcon -R -t httpd_sys_content_t /var/www/digitaltrustinfrastructure.org 2>/dev/null || true
```

Comprueba que hay contenido:

```bash
ls -la /var/www/digitaltrustinfrastructure.org/index.html
ls /var/www/digitaltrustinfrastructure.org/ | head
```
...
Plantilla: `[nginx-dti-site.conf](nginx-dti-site.conf)` - un solo bloque `listen 80`, sin SSL.
...
odtis/scripts/nginx-dti-site.conf \
ec2-user@TU_IP_EC2:/tmp/digitaltrustinfrastructure.org.conf
```

En el EC2, **comprueba que el archivo llegó**:

```bash
ls -la /tmp/digitaltrustinfrastructure.org.conf
```
...
```bash
sudo cp /tmp/digitaltrustinfrastructure.org.conf /etc/nginx/conf.d/digitaltrustinfrastructure.org.conf
sudo nginx -t
...
```bash
sudo cp /tmp/digitaltrustinfrastructure.org.conf /etc/nginx/sites-available/digitaltrustinfrastructure.org
sudo ln -sf /etc/nginx/sites-available/digitaltrustinfrastructure.org /etc/nginx/sites-enabled/digitaltrustinfrastructure.org
sudo nginx -t
sudo systemctl reload nginx
```

> **Error `mv: ... No such file or directory`:** casi siempre falta el directorio destino (`sites-available` no existe en tu AMI). Usa **Opción A** o crea las carpetas en **Opción B**. Preferible `cp` en lugar de `mv` por si quieres conservar `/tmp/digitaltrustinfrastructure.org.conf`.

**Importante:**

- No toques el vhost de `your-other-domain.example`.
- `server_name digitaltrustinfrastructure.org www.digitaltrustinfrastructure.org;` y `root /var/www/digitaltrustinfrastructure.org;` exclusivos de ODTIS.
- MkDocs requiere `try_files $uri $uri/ $uri/index.html =404;` (incluido en la plantilla).

Prueba en el servidor:

```bash
curl -sI -H "Host: digitaltrustinfrastructure.org" http://127.0.0.1/ | head -3
...
"$ODTIS_EC2_HOST:/var/www/digitaltrustinfrastructure.org/"
...
| Paso                           | Comando / URL                                      |
| ------------------------------ | -------------------------------------------------- |
| Archivos                       | `ls /var/www/digitaltrustinfrastructure.org/index.html`                 |
| Origen HTTP                    | `curl -sI -H "Host: digitaltrustinfrastructure.org" http://127.0.0.1/`  |
| Público (HTTPS vía Cloudflare) | [https://digitaltrustinfrastructure.org](https://digitaltrustinfrastructure.org)             |
...
| **404** en rutas                                  | Falta `try_files`                                       | Usar `[nginx-dti-site.conf](nginx-dti-site.conf)`                 |
| **403 Forbidden**                                 | Carpeta vacía (rsync falló) o nginx/SELinux sin lectura | Ver sección 7 abajo                                                 |
| `**Permission denied` en rsync**                  | `/var/www/digitaltrustinfrastructure.org` es de `root`                       | `sudo chown -R ec2-user:ec2-user /var/www/digitaltrustinfrastructure.org`                |
| `**mv: No such file or directory`**               | No existe `sites-available/` (Amazon Linux)             | Usar `/etc/nginx/conf.d/digitaltrustinfrastructure.org.conf` (sección 4.2)               |
| Otro vhost roto                                   | Vhost sobrescrito                                       | Un archivo por dominio; restaurar backup                            |
| Página vieja                                      | Caché Cloudflare                                        | Purge cache                                                         |
| Enlaces mal dominio                               | Build sin `site_url`                                    | `https://digitaltrustinfrastructure.org` en `mkdocs.yml` + rebuild                       |
...
Si `ls /var/www/digitaltrustinfrastructure.org/index.html` falla **y** `curl` devuelve **403**:

1. **No hay sitio desplegado** (el rsync anterior falló por permisos).
2. Nginx sí reconoce el vhost `digitaltrustinfrastructure.org` (por eso responde 403, no 404 de otro sitio).
...
../build/odtis-spec-site/ \
ec2-user@YOUR_EC2_PUBLIC_IP:/var/www/digitaltrustinfrastructure.org/
```
...
sudo find /var/www/digitaltrustinfrastructure.org -type d -exec chmod 755 {} \;
sudo find /var/www/digitaltrustinfrastructure.org -type f -exec chmod 644 {} \;
sudo chcon -R -t httpd_sys_content_t /var/www/digitaltrustinfrastructure.org
```

### Paso D - verificar vhost apunta al docroot correcto

```bash
sudo nginx -T 2>/dev/null | grep -A5 'server_name digitaltrustinfrastructure.org'
# root debe ser /var/www/digitaltrustinfrastructure.org;
```
...
```bash
ls -la /var/www/digitaltrustinfrastructure.org/index.html
curl -sI -H "Host: digitaltrustinfrastructure.org" http://127.0.0.1/ | head -5
# HTTP/1.1 200 OK
```
...
```bash
curl -sI -H "Host: digitaltrustinfrastructure.org" http://127.0.0.1/ # → 200 OK
```

pero **[https://digitaltrustinfrastructure.org](https://digitaltrustinfrastructure.org)** en el navegador no carga, el origen (Nginx + archivos) está bien. El fallo está en **DNS o Cloudflare** (camino Internet → Cloudflare → EC2).

### Desde tu Mac

```bash
# ¿Resuelve el dominio?
dig digitaltrustinfrastructure.org +short
dig www.digitaltrustinfrastructure.org +short

# ¿Apunta a Cloudflare o está vacío?
dig digitaltrustinfrastructure.org NS +short

# Prueba HTTPS pública
curl -sI --max-time 10 https://digitaltrustinfrastructure.org | head -10
curl -sI --max-time 10 https://digitaltrustinfrastructure.org | head -10

# Prueba IP directa (bypass DNS) - debe dar 200 si Nginx está bien
curl -sI --max-time 10 -H "Host: digitaltrustinfrastructure.org" http://YOUR_EC2_PUBLIC_IP/ | head -5
```


| Resultado                                                           | Significado                                             | Acción                                                     |
| ------------------------------------------------------------------- | ------------------------------------------------------- | ---------------------------------------------------------- |
| `dig digitaltrustinfrastructure.org` **vacío** o IP incorrecta                           | DNS no configurado o NS del registrar no son Cloudflare | Ver checklist Cloudflare abajo                             |
| `dig` devuelve IPs **104.x / 172.x** (Cloudflare)                   | DNS edge OK                                             | Revisar registro A en dashboard (IP origen) y SSL Flexible |
| `curl https://digitaltrustinfrastructure.org` **timeout / no resuelve**                  | DNS o zona inactiva en Cloudflare                       | Añadir sitio en Cloudflare; cambiar NS en Namecheap        |
| `curl` a IP directa con `Host: digitaltrustinfrastructure.org` → **200** pero dominio no | Solo falla DNS/Cloudflare                               | Arreglar registros en Cloudflare                           |
| **522** en navegador                                                | Cloudflare no alcanza el EC2                            | Security group puerto **80**; Nginx activo                 |
| **525 / 526**                                                       | Modo SSL distinto a Flexible                            | SSL/TLS → **Flexible** en zona `digitaltrustinfrastructure.org`                 |


### Checklist Cloudflare (zona `digitaltrustinfrastructure.org`)

1. **Websites** → `digitaltrustinfrastructure.org` aparece como **Active** (no solo `your-other-domain.example`).
...
3. **SSL/TLS → Edge Certificates:** certificado activo para `digitaltrustinfrastructure.org` (suele generarse en minutos).
...
1. Cloudflare → sitio **digitaltrustinfrastructure.org** → copia los **2 nameservers** exactos (Overview).
2. Namecheap → **Domain List** → `digitaltrustinfrastructure.org` → **Manage**.
3. Sección **Nameservers** → elige **Custom DNS** (no "Namecheap BasicDNS").
4. Pega los 2 NS de Cloudflare → **Save** (checkmark verde).
5. Espera 15-60 min; comprueba hasta que `dig digitaltrustinfrastructure.org NS +short` muestre `*.cloudflare.com`.
...
1. **DNS → Records:** `A` `@` → `YOUR_EC2_PUBLIC_IP` Proxied; `CNAME` `www` → `digitaltrustinfrastructure.org` Proxied.
...
3. `A` o CNAME `www` → `YOUR_EC2_PUBLIC_IP` / `digitaltrustinfrastructure.org`.

`https://digitaltrustinfrastructure.org` funcionará en minutos; HTTPS público requiere Cloudflare o cert en origen.

### DNS en propagación: curl no responde pero el sitio ya está live

Síntoma: `dig digitaltrustinfrastructure.org +short @1.1.1.1` muestra IPs Cloudflare (`104.x` / `172.x`) y **200 OK**, pero en tu Mac:

```bash
dig digitaltrustinfrastructure.org +short
# 192.64.119.26 ← parking Namecheap (MAL)

curl -sI https://digitaltrustinfrastructure.org
...
```bash
curl -sI --resolve digitaltrustinfrastructure.org:443:104.21.6.193 https://digitaltrustinfrastructure.org | head -5
# HTTP/2 200
```
...
1. Cierra el navegador por completo; ventana privada → [https://digitaltrustinfrastructure.org](https://digitaltrustinfrastructure.org)
2. Comprueba:
...
curl -sI https://digitaltrustinfrastructure.org | head -5
# HTTP/2 200
```
...
- Ventana privada, o
- `https://digitaltrustinfrastructure.org/?nocache=1`
- Cloudflare → **Caching → Purge Everything** tras el primer deploy

### Si el dominio es nuevo

La propagación de NS puede tardar **minutos a 48 h**. Mientras tanto `dig digitaltrustinfrastructure.org` puede estar vacío aunque el servidor responda bien en localhost.
...
| Nginx (Flexible) | `odtis/scripts/nginx-dti-site.conf` |
| MkDocs           | `odtis/site/mkdocs.yml`              |


---

*Junio 2026 - EC2 + Cloudflare SSL **Flexible** · your-other-domain.example + digitaltrustinfrastructure.org*